#ifndef CAFFE_BALANCED_SIGMOID_CROSS_ENTROPY_LOSS_REF_LAYER_HPP_
#define CAFFE_BALANCED_SIGMOID_CROSS_ENTROPY_LOSS_REF_LAYER_HPP_
...
/**
 * @brief CPU only for test. Computes the BALANCED cross-entropy (logistic) loss @f$
 *          E = \frac{-1}{n} \sum\limits_{n=1}^N \left[
 *                  \frac{n_{neg}}{n} p_n \log \hat{p}_n +
 *                  \frac{n_{pos}}{n} (1 - p_n) \log(1 - \hat{p}_n)
 *              \right]
...
class BalancedSigmoidCrossEntropyLossRefLayer : public LossLayer<Dtype> {
 public:
  explicit BalancedSigmoidCrossEntropyLossRefLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "BalancedSigmoidCrossEntropyLossRef"; }

 protected:
  /// @copydoc BalancedSigmoidCrossEntropyLossRefLayer
  ...
  // loss weight for pos/neg labels
  Blob<Dtype> loss_pw_, loss_nw_;
};

}  // namespace caffe

#endif  // CAFFE_BALANCED_SIGMOID_CROSS_ENTROPY_LOSS_REF_LAYER_HPP_
