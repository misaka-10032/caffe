...
template <typename Dtype>
void HoughTransposeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                            const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(bottom.size(), 1) << "Only 1 bottom is allowed.";
  CHECK_EQ(top.size(), 1) << "Only 1 top is allowed.";
  CHECK_EQ(bottom[0]->shape(0), 1) << "Bach size should be 1";
  CHECK_EQ(bottom[0]->shape(1), 1) << "Channel size should be 1";
  THETA_ = bottom[0]->shape(2);
  RHO_ = bottom[0]->shape(3);
  H_ = this->layer_param_.hough_param().h_range();
  W_ = this->layer_param_.hough_param().w_range();
  vector<int> hough_shape(4);
  hough_shape[0] = H_;
  hough_shape[1] = W_;
  hough_shape[2] = THETA_;
...
template <typename Dtype>
void HoughTransposeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                         const vector<Blob<Dtype>*>& top) {
  vector<int> top_shape(4);
  top_shape[0] = 1;
  top_shape[1] = 1;
...
template <typename Dtype>
void HoughTransposeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                             const vector<Blob<Dtype>*>& top) {
  caffe_csrmv(CblasNoTrans, hb_ptr_->H()*hb_ptr_->W(), hb_ptr_->RHO()*hb_ptr_->THETA(),
              Dtype(1), hb_ptr_->csr_val_cpu_data(), hb_ptr_->csr_ro_cpu_data(),
              hb_ptr_->csr_ci_cpu_data(), bottom[0]->cpu_data(), Dtype(0),
              top[0]->mutable_cpu_data());
}

template <typename Dtype>
void HoughTransposeLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
                                              const vector<bool>& propagate_down,
                                              const vector<Blob<Dtype>*>& bottom) {
  if (!propagate_down[0])
    return;
  ...
  caffe_csrmv(CblasNoTrans, hb_ptr_->RHO()*hb_ptr_->THETA(), hb_ptr_->H()*hb_ptr_->W(),
              Dtype(1), hb_ptr_->csc_val_cpu_data(), hb_ptr_->csc_co_cpu_data(),
              hb_ptr_->csc_ri_cpu_data(), top[0]->cpu_diff(), Dtype(0),
              bottom[0]->mutable_cpu_diff());
}

INSTANTIATE_CLASS(HoughTransposeLayer);
REGISTER_LAYER_CLASS(HoughTranspose);
